...
class PlannerAgent:
    def create_daily_spot_plan(
        ...
        if llm_provider and hasattr(llm_provider, "draft_itinerary"):
            ...
            if normalized:
                return self._ensure_daily_density(normalized, ranked_pois, request, constraints), "百炼生成初版多日路线"
        return self._ensure_daily_density(self._heuristic_daily_spot_plan(request, ranked_pois, constraints), ranked_pois, request, constraints), "本地规则生成初版多日路线"
    ...
    def revise_daily_spot_plan(
        ...
        if llm_provider and hasattr(llm_provider, "revise_itinerary"):
            ...
            if normalized:
                return self._ensure_daily_density(normalized, ranked_pois, request, constraints), "百炼根据校验结果完成路线修正"
        return self._ensure_daily_density(self._heuristic_revision(request, ranked_pois, constraints), ranked_pois, request, constraints), "本地规则完成路线修正"
    ...
    def _heuristic_daily_spot_plan(
        ...
        base_slots_per_day = min({"relaxed": 2, "balanced": 3, "dense": 4}[request.style], constraints.max_daily_spots)
        selected_count = min(len(ranked_pois), max(request.days * base_slots_per_day, request.days))
    ...
    def _build_notes(self, spots: list[PointOfInterest], style: str) -> list[str]:
        ...
        if style == "relaxed":
            ...
            notes.append("当天节奏较紧，建议提前出发，并优先使用地铁或打车缩短通勤时间。")
        else:
    ...
    def _ensure_daily_density(
        ...
        if not daily_plans:
            return daily_plans
        target_per_day = min({"relaxed": 2, "balanced": 3, "dense": 4}[request.style], constraints.max_daily_spots)
        if target_per_day <= 1:
            return daily_plans
    ...
    def _prefer_dense_core(
        ...
        if not ranked_pois:
            return []
        target_count = min(len(ranked_pois), max(days, days * max(1, min(3, max_daily_spots))))
        seed = ranked_pois[0]
        scored = sorted(
            ranked_pois,
            key=lambda poi: (
                self._distance(seed, poi) + (12.0 if self._distance(seed, poi) > 20.0 else 4.0 if self._distance(seed, poi) > 10.0 else 0.0),
                poi.ticket_cost,
            ),
        )
